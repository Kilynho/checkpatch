...
def _format_timestamp():
    """Genera timestamp formateado para reportes HTML."""
...
def _safe_id(text):
    """Genera un ID único y seguro para HTML basado en texto."""
    h = hashlib.sha1(text.encode("utf-8")).hexdigest()[:12]
    return f"id_{h}"

def _generate_html_header(title, timestamp):
    """
    Genera el header HTML común con CSS.
    ...
    lines = [
        ...
        COMMON_CSS,
        "</style></head><body>",
        f"<h1>{title} <span style='font-weight:normal'>{timestamp}</span></h1>"
    ]
    return lines

def _generate_percentage_bar(count, total, max_width=170, css_class=""):
    """
    Genera HTML para una barra de progreso con porcentaje.
    
    Args:
        count: Valor actual
        total: Valor total
        max_width: Ancho máximo de la barra en pixels
        css_class: Clase CSS para colorear la barra (bar-errors, bar-warnings, etc.)
        
    Returns:
        Tupla (pct_text, bar_width_px, pct_value) donde:
        - pct_text: String del porcentaje formateado (ej: "85.5%")
        - bar_width_px: Ancho de la barra en pixels
        - pct_value: Valor numérico del porcentaje (0-100)
    """
    pct_text = percentage(count, total)
    pct_value = percentage_value(count, total)
    bar_width_px = bar_width(count, total, max_width=max_width)
    return pct_text, bar_width_px, pct_value

def _format_diff_html(diff_text):
    """
    Formatea un diff unificado con colores HTML.
    
    Args:
        diff_text: Texto del diff en formato unificado
        
    Returns:
        String HTML con el diff coloreado
    """
...
def _get_diff(bak_path, current_path):
    """
    Obtiene el diff entre dos archivos usando el comando diff.
    
    Args:
        bak_path: Ruta del archivo backup
        current_path: Ruta del archivo actual
        
    Returns:
        String con el diff o string vacío en caso de error
    """
    try:
        res = subprocess.run(['diff', '-u', bak_path, current_path], 
                           capture_output=True, text=True)
        return res.stdout
    except Exception:
        return ''

def _colorize_checkpatch_output(text):
    """
    Coloriza el output del checkpatch: ERROR en rojo, WARNING en naranja, + en verde, # en gris.
    
    Args:
        text: Salida de checkpatch
        
    Returns:
        String HTML con colores aplicados
    """
    lines = text.split('\n')
    colored_lines = []
    for line in lines:
...
def generate_html_report(report_data, html_file, kernel_dir="."):
    """
    Genera reporte HTML de autofix con resumen global y detalle por motivo/fichero.
    
    Args:
        report_data: Dict con datos de fixes aplicados {filepath: {error: [...], warning: [...]}}
        html_file: Ruta del archivo HTML de salida
        kernel_dir: Directorio raíz del kernel (para rutas relativas)
    """
...
def generate_analyzer_html(analysis_data, html_file):
    ...
    if warning_reasons:
        ...
        for reason, count in sorted(warning_reasons.items(), key=lambda x: -x[1]):
            ...
            reason_id = _safe_id("WARNING:" + reason)
...
def generate_detail_file_html(analysis_data, html_file):
    ...
    for fp in sorted(file_issues.keys()):
        ...
        if fp in file_outputs:
            ...
            colored_output = _colorize_checkpatch_output(output)
...
def generate_compile_html(results, html_file, kernel_root=None):
    ...
    # CSS adicional específico para compilación
    append("""
    .success-box { 
        background: #e8f5e9; 
